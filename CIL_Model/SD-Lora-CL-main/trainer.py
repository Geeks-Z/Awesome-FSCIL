...
def _resolve_inference_added_params_m(model, args, final=False):
    embed_dim = 1024 if "large" in args.get("backbone_type", "").lower() else 768
    depth = 24 if "large" in args.get("backbone_type", "").lower() else 12
    rank = getattr(getattr(model._network, "backbone", None), "rank", 10)
    num_tasks = args.get("nb_tasks", 1) if final else max(getattr(model, "_cur_task", -1) + 1, 1)
    nb_classes = args.get("nb_classes", args.get("init_cls", 0)) if final else getattr(model, "_total_classes", args.get("init_cls", 0))
    return (4 * embed_dim * rank * depth * num_tasks + nb_classes * (embed_dim + 1)) / 1e6
...
def _print_inference_metrics(stage_metrics, final_inference_added_m=None):
    ...
    if final_inference_added_m is not None:
        print(f"{'Final':<8}{final_inference_added_m:<12.2f}")


def print_stage_metric_tables(stage_metrics, final_trainable_total_m, final_inference_added_m=None):
    ...
    print("-" * 90)
    _print_inference_metrics(stage_metrics, final_inference_added_m)
    print("=" * 90 + "\n")
...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        infer_params_total_m = _resolve_inference_added_params_m(model, args, final=False)
    ...
    final_inference_added_m = _resolve_inference_added_params_m(model, args, final=True)
    print_stage_metric_tables(stage_metrics, final_trainable_total_m, final_inference_added_m)
