...
def get_backbone(args, pretrained=False):
    ...
    # ViT-Large
        ...
        if args["model_name"] == "memo":
            ...
            _basenet, _adaptive_net = timm.create_model(
                "vit_base_patch16_224_memo", pretrained=True, num_classes=0
            )
    ...
    # Coda_Prompt
        ...
        if args["model_name"] == "coda_prompt":
            ...
            model = timm.create_model(
                args["backbone_type"], pretrained=args["pretrained"]
            )
...
class IncrementalNet(BaseNet):
    ...
    def load_fc(self, task_id):
        ...
        self.fc = self.generate_fc(self.feature_dim, (task_id + 1) * 10).to(
            self._device
        )
        for i in range(task_id + 1):
            for j in range(i, task_id + 1):
                ...
                if j == i:
                    self.fc.weight.data[i * 10 : i * 10 + 10] = temp_weights.data[
                        i * 10 : i * 10 + 10
                    ]
                    self.fc.bias.data[i * 10 : i * 10 + 10] = temp_bias.data[
                        i * 10 : i * 10 + 10
                    ].to(self._device)
                else:
                    self.fc.weight.data[i * 10 : i * 10 + 10] += temp_weights.data[
    ...
    def construct_dual_branch_network(self, tuned_model):
        if "ssf" in self.args["backbone_type"]:
            ...
            self.backbones.append(
                get_backbone(newargs)
            )  # pretrained model without scale
