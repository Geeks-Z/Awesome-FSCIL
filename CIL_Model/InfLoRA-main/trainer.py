...
def _resolve_inference_added_params_m(model, args):
    prefixes = ("classifier_pool", "lora_A_k", "lora_B_k", "lora_A_v", "lora_B_v")
...
def _train(args):
    backbone_tag = args.get("backbone_type", "vit_base_patch16_224").replace("/", "-")
    ...
    for device in args["device"]:
        if isinstance(device, torch.device) and device.type == "cuda":
            gpu_names.append(torch.cuda.get_device_name(device))
    args["gpu_models"] = gpu_names
    _set_random(args)
    print_args(args)
