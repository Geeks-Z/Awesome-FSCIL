...
class Attention_LoRA(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., r=64, n_tasks=10):
        ...
        self.lora_A_k_2 = nn.ModuleList([nn.Linear(dim, r, bias=False) for _ in range(n_tasks)])
        self.lora_B_k_2 = nn.ModuleList([nn.Linear(r, dim, bias=False) for _ in range(n_tasks)])
        self.lora_A_v_2 = nn.ModuleList([nn.Linear(dim, r, bias=False) for _ in range(n_tasks)])
        self.lora_B_v_2 = nn.ModuleList([nn.Linear(r, dim, bias=False) for _ in range(n_tasks)])
    ...
    def init_param(self):
        for t in range(len(self.lora_A_k)):
            ...
            # nn.init.kaiming_uniform_(self.lora_A_k_2[t].weight, a=math.sqrt(5))
            # nn.init.kaiming_uniform_(self.lora_A_v_2[t].weight, a=math.sqrt(5))
            self.lora_A_k_2[t].weight.data = copy.deepcopy(self.lora_A_k[t].weight.data)
            self.lora_A_v_2[t].weight.data = copy.deepcopy(self.lora_A_v[t].weight.data)
            kr = 1.0 / self.dim
            nn.init.sparse_(self.lora_B_k[t].weight, sparsity=(1 - kr))
            nn.init.sparse_(self.lora_B_v[t].weight, sparsity=(1 - kr))
            # nn.init.sparse_(self.lora_B_k_2[t].weight, sparsity=(1 - kr))
            # nn.init.sparse_(self.lora_B_v_2[t].weight, sparsity=(1 - kr))
            self.lora_B_k_2[t].weight.data = copy.deepcopy(self.lora_B_k[t].weight.data)
            self.lora_B_v_2[t].weight.data = copy.deepcopy(self.lora_B_v[t].weight.data)
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_cur_x=False,first_flow=0,scape=False,scape_params=None):
        B, N, C = x.shape
        ...
        if task > -0.5:
            if scape:
                weight_k_1 = torch.stack(
                    [torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) for t in range(task + 1)],
                    dim=0).sum(dim=0)
                weight_v_1 = torch.stack(
                    [torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight) for t in range(task + 1)],
                    dim=0).sum(dim=0)

                weight_k_2 = torch.stack(
                    [torch.mm(self.lora_B_k_2[t].weight, self.lora_A_k_2[t].weight) for t in range(task + 1)],
                    dim=0).sum(dim=0)
                weight_v_2 = torch.stack(
                    [torch.mm(self.lora_B_v_2[t].weight, self.lora_A_v_2[t].weight) for t in range(task + 1)],
                    dim=0).sum(dim=0)

                weight_k = scape_params['scale_1'] * weight_k_1 + scape_params['scale_2'] * weight_k_2
                weight_v = scape_params['scale_1'] * weight_v_1 + scape_params['scale_2'] * weight_v_2

            else:
                if first_flow == 0: #**************** 真正适配器合并之后的推理
                    weight_k = self.lora_weight_k_meg
                    weight_v = self.lora_weight_v_meg
                elif first_flow == 90: #****************特殊流程: 真正合并适配器之前，用适配器平均替代，在 Base Train的过程中使用
                    weight_k_1 = torch.stack(
                        [torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight) for t in range(task + 1)],
                        dim=0).sum(dim=0)
                    weight_v_1 = torch.stack(
                    ...
                    weight_v_2 = torch.stack(
                        [torch.mm(self.lora_B_v_2[t].weight, self.lora_A_v_2[t].weight) for t in range(task + 1)],
                        dim=0).sum(dim=0)
                    weight_k = (weight_k_1 + weight_k_2) * 0.5
                    weight_v = (weight_v_1 + weight_v_2) * 0.5
                elif first_flow == 91: #****************特殊流程: 合并适配器过程中的推理
                    weight_k = self.lora_weight_k_1.weight
                    weight_v = self.lora_weight_v_1.weight
                elif first_flow == 92: #****************特殊流程: 合并适配器过程中的推理
                    weight_k = self.lora_weight_k_2.weight
                    weight_v = self.lora_weight_v_2.weight
                elif first_flow == 1: # 无边距的模型推理
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False, get_cur_x=False,first_flow=0,scape=False,scape_params=None):
        x = x + self.drop_path1(self.ls1(
            self.attn(self.norm1(x), task, register_hook=register_hook, get_feat=get_feat, get_cur_feat=get_cur_feat,
                      get_cur_x=get_cur_x,first_flow=first_flow,scape=scape,scape_params=scape_params)))
